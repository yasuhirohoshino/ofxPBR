	...
	PBR() {
		...
		void main() {
			if (renderForDepthMap == true) {
				...
				gl_Position = viewMat * positionVarying;
			}
			else {
				rawPositionVarying = (inverse(viewMatrix) * modelViewMatrix) * position;
		...
		float CalcHardShadow(int index) {
			...
			for (int j = 0; j < 8; j++) {
				if (currentDepth - lights[index].bias > texture(shadowMap, vec3(projCoords.xy + poissonDisk[j] * (1.0 / (depthMapRes)), index)).r) {
					visiblity -= 1.0 / 9.0;
				}
			}
			if (projCoords.x >= 1.0 || projCoords.x <= 0.0 ||
		...
		float CalsOmniShadow(vec3 fragPos)
		{
			float shadow = 0.0;
			float farPlane = 5000.0;
			vec3 fragToLight = fragPos - vec3(0, 100, 100);// lights[0].position;
		...
		void LightWithShadow(vec4 v_positionVarying, vec3 v_normalVarying, vec3 color, float roughness, int index, out vec3 deffuse, out vec3 specular) {
			...
			shadow = CalsOmniShadow(rawPositionVarying.xyz);
